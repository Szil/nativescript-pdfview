...
import { PDFViewCommon, srcProperty, defaultpageProperty, bookmarkPathProperty, bookmarkLabelProperty, BookmarkCommon, ControllerRect } from './common';
...
export class PDFViewer extends PDFViewCommon {
  nativeView: PDFView;
    ...
    return Promise.resolve()
      .then(() => {
        ...
        if (document) {
          ...
          console.log(`PDFView ios (Step 3) PDF loaded (Version: ${document.majorVersion}.${document.minorVersion}), Fixed local path: ` + src);

          // allow load to be done
          return that.setTimeout(1);
        } else {
          console.error('PDFView ios - could not load pdf file: ' + url);
      ...
      .then(() => {
        if (that.value_bookmark_label) {
          console.log('PDFView ios (Step 4) Go to outlinelabel: ', src, that.value_bookmark_label);
          that.goToBookmarkByLabel('' + that.value_bookmark_label);
        } else if (that.value_bookmark_path) {
          console.log('PDFView ios (Step 4) Go to outlinepath: ', src, that.value_bookmark_path);
          that.goToBookmarkByPath(that.value_bookmark_path);
        } else {
          console.log('PDFView ios (Step 4) Go to page: ', src, that.value_default_page);
