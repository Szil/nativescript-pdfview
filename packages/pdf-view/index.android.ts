...
import { PDFViewCommon, srcProperty, defaultpageProperty, BookmarkCommon, bookmarkPathProperty, bookmarkLabelProperty, ControllerRect } from './common';
...
export class PDFView extends PDFViewCommon {
  ...
  public onLoaded(): void {
    ...
    this.loadPDF(this.value_src).catch((err) => {
      ...
      PDFViewCommon.notifyOfEvent(PDFViewCommon.errorEvent, pdfViewRef);
    });
  }
  ...
  private loadPDFInternal(uri: android.net.Uri, src: string, default_page: number) {
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      this.android.fromUri(uri).onLoad(onLoadHandler).defaultPage(default_page).load();
    });
  }
